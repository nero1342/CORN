...
class SwinTransformer(nn.Module):
    def __init__(self,
        ...
        if weights:
            logger.info(f"Loading backbone weights from {weights}")
            _incompatible_keys = self.load_state_dict(torch.load(weights, map_location=next(self.parameters()).device)['model'], strict=False)
