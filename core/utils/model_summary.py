""" https://espnet.github.io/espnet/_modules/espnet2/torch_utils/model_summary.html """
...
def model_summary(model: torch.nn.Module) -> str:
    ...
    percent_trainable = "{:.3f}".format(num_params * 100.0 / tot_params)
    tot_params = get_human_readable_count(tot_params)
    num_params = get_human_readable_count(num_params)

    summary = "\n\nModel summary:\n"
    summary += f"    Class Name: [bold italic yellow on red blink]{model.__class__.__name__}[/]\n"
    summary += f"    Total Number of model parameters: {tot_params}\n"
    summary += (
        f"    Number of trainable parameters: {num_params} ({percent_trainable}%)\n"
    )
    num_bytes = humanfriendly.format_size(
        sum(
            p.numel() * to_bytes(p.dtype) for p in model.parameters()
        )
    )
    summary += f"    Total estimated model params size: {num_bytes}\n"
